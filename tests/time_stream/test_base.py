...
class TestSelectColumns(unittest.TestCase):
    ...
    def test_select_column_doesnt_mutate_original_tf(self) -> None:
        ...
        self.assertEqual(col1_tf, expected)
        ...
        self.assertEqual(col2_tf, expected)
        assert_frame_equal(tf.df, original_df)
...
class TestColumnMetadata(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.tf = TimeFrame(self.df, time_name="time").with_column_metadata(self.metadata)
    ...
    def test_all_columns_have_metadata(self) -> None:
        ...
        self.assertEqual(set(self.df.columns), set(self.tf.column_metadata.keys()))
        self.assertEqual(self.tf.column_metadata["col1"], {"key1": "1", "key2": "10", "key3": "100", "key4": "1000"})
        self.assertEqual(self.tf.column_metadata["col2"], {"key1": "2", "key2": "20", "key3": "200"})
        self.assertEqual(self.tf.column_metadata["col3"], {})
    ...
    def test_set_column_metadata(self) -> None:
        ...
        self.assertEqual(self.tf.column_metadata, column_metadata)
...
class TestMetadata(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.tf = TimeFrame(self.df, time_name="time").with_metadata(self.metadata)
...
class TestInitFlagColumn(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.tf = TimeFrame(df=self.df, time_name="time")
        self.tf.register_flag_system("quality_control", self.flag_system)
    ...
    def test_init_flag_column_success(self) -> None:
        ...
        self.assertEqual(self.tf._flag_manager._flag_columns, {})

        self.tf.init_flag_column("value", "quality_control", "flag_column")
        self.assertEqual(
            self.tf._flag_manager._flag_columns,
            {"flag_column": FlagColumn("flag_column", "value", self.flag_system)},
        )
        self.assertIn("flag_column", self.tf.flag_columns)
    ...
    def test_init_column_adds_to_metadata(self) -> None:
        ...
        self.assertEqual(self.tf.column_metadata, expected)
    ...
    def test_init_flag_column_with_single_value(self) -> None:
        ...
        self.tf.init_flag_column("value", "quality_control", "flag_column", 1)
        expected_values = pl.Series("flag_column", [1, 1, 1], dtype=pl.Int64)
        assert_series_equal(self.tf.df["flag_column"], expected_values)
    ...
    def test_init_flag_column_with_list_value(self) -> None:
        ...
        self.tf.init_flag_column("value", "quality_control", "flag_column", [1, 2, 4])
    ...
    def test_with_no_flag_column_name(self) -> None:
        ...
        self.tf.init_flag_column("value", "quality_control")
        ...
        self.assertIn(default_name, self.tf.flag_columns)
...
class TestTimeSeriesEquality(unittest.TestCase):
    ...
    def test_equal_timeseries(self) -> None:
        ...
        self.assertEqual(self.tf_original, tf_same)
    ...
    def test_different_data_values(self) -> None:
        ...
        self.assertNotEqual(self.tf_original, tf_different_df)

        # Test the expected property has changed
        assert_frame_not_equal(self.tf_original.df, tf_different_df.df)
        self.assertEqual(self.tf_original.time_name, tf_different_df.time_name)
        self.assertEqual(self.tf_original.resolution, tf_different_df.resolution)
        self.assertEqual(self.tf_original.periodicity, tf_different_df.periodicity)
        self.assertEqual(self.tf_original.time_anchor, tf_different_df.time_anchor)
        self.assertEqual(self.tf_original._flag_manager, tf_different_df._flag_manager)
        self.assertEqual(self.tf_original.metadata, tf_different_df.metadata)
        self.assertEqual(self.tf_original.column_metadata, tf_different_df.column_metadata)
    ...
    def test_different_time_values(self) -> None:
        ...
        self.assertNotEqual(self.tf_original, tf_different_times)

        # Test the expected property has changed
        assert_frame_not_equal(self.tf_original.df, tf_different_times.df)
        self.assertEqual(self.tf_original.time_name, tf_different_times.time_name)
        self.assertEqual(self.tf_original.resolution, tf_different_times.resolution)
        self.assertEqual(self.tf_original.periodicity, tf_different_times.periodicity)
        self.assertEqual(self.tf_original.time_anchor, tf_different_times.time_anchor)
        self.assertEqual(self.tf_original._flag_manager, tf_different_times._flag_manager)
        self.assertEqual(self.tf_original.metadata, tf_different_times.metadata)
        self.assertEqual(self.tf_original.column_metadata, tf_different_times.column_metadata)
    ...
    def test_different_time_name(self) -> None:
        ...
        self.assertNotEqual(self.tf_original, tf_different_time_name)

        # Test the expected property has changed
        assert_frame_not_equal(self.tf_original.df, tf_different_time_name.df)
        self.assertNotEqual(self.tf_original.time_name, tf_different_time_name.time_name)
        self.assertEqual(self.tf_original.resolution, tf_different_time_name.resolution)
        self.assertEqual(self.tf_original.periodicity, tf_different_time_name.periodicity)
        self.assertEqual(self.tf_original.time_anchor, tf_different_time_name.time_anchor)
        self.assertEqual(self.tf_original._flag_manager, tf_different_time_name._flag_manager)
        self.assertEqual(self.tf_original.metadata, tf_different_time_name.metadata)
        self.assertNotEqual(self.tf_original.column_metadata, tf_different_time_name.column_metadata)
    ...
    def test_different_periodicity(self) -> None:
        ...
        self.assertNotEqual(self.tf_original, tf_different_periodicity)

        # Test the expected property has changed
        assert_frame_equal(self.tf_original.df, tf_different_periodicity.df)
        self.assertEqual(self.tf_original.time_name, tf_different_periodicity.time_name)
        self.assertEqual(self.tf_original.resolution, tf_different_periodicity.resolution)
        self.assertNotEqual(self.tf_original.periodicity, tf_different_periodicity.periodicity)
        self.assertEqual(self.tf_original.time_anchor, tf_different_periodicity.time_anchor)
        self.assertEqual(self.tf_original._flag_manager, tf_different_periodicity._flag_manager)
        self.assertEqual(self.tf_original.metadata, tf_different_periodicity.metadata)
        self.assertEqual(self.tf_original.column_metadata, tf_different_periodicity.column_metadata)
    ...
    def test_different_resolution(self) -> None:
        ...
        self.assertNotEqual(self.tf_original, tf_different_resolution)

        assert_frame_equal(self.tf_original.df, tf_different_resolution.df)
        self.assertEqual(self.tf_original.time_name, tf_different_resolution.time_name)
        self.assertNotEqual(self.tf_original.resolution, tf_different_resolution.resolution)
        self.assertEqual(self.tf_original.periodicity, tf_different_resolution.periodicity)
        self.assertEqual(self.tf_original.time_anchor, tf_different_resolution.time_anchor)
        self.assertEqual(self.tf_original._flag_manager, tf_different_resolution._flag_manager)
        self.assertEqual(self.tf_original.metadata, tf_different_resolution.metadata)
        self.assertEqual(self.tf_original.column_metadata, tf_different_resolution.column_metadata)
    ...
    def test_different_time_anchor(self) -> None:
        ...
        self.assertNotEqual(self.tf_original, tf_different_time_anchor)

        # Test the expected property has changed
        assert_frame_equal(self.tf_original.df, tf_different_time_anchor.df)
        self.assertEqual(self.tf_original.time_name, tf_different_time_anchor.time_name)
        self.assertEqual(self.tf_original.resolution, tf_different_time_anchor.resolution)
        self.assertEqual(self.tf_original.periodicity, tf_different_time_anchor.periodicity)
        self.assertNotEqual(self.tf_original.time_anchor, tf_different_time_anchor.time_anchor)
        self.assertEqual(self.tf_original._flag_manager, tf_different_time_anchor._flag_manager)
        self.assertEqual(self.tf_original.metadata, tf_different_time_anchor.metadata)
        self.assertEqual(self.tf_original.column_metadata, tf_different_time_anchor.column_metadata)
    ...
    def test_different_flag_systems(self) -> None:
        ...
        self.assertNotEqual(self.tf_original, tf_different_flags)

        assert_frame_equal(self.tf_original.df, tf_different_flags.df)
        self.assertEqual(self.tf_original.time_name, tf_different_flags.time_name)
        self.assertEqual(self.tf_original.resolution, tf_different_flags.resolution)
        self.assertEqual(self.tf_original.periodicity, tf_different_flags.periodicity)
        self.assertEqual(self.tf_original.time_anchor, tf_different_flags.time_anchor)
        self.assertNotEqual(self.tf_original._flag_manager, tf_different_flags._flag_manager)
        self.assertEqual(self.tf_original.metadata, tf_different_flags.metadata)
        self.assertEqual(self.tf_original.column_metadata, tf_different_flags.column_metadata)
    ...
    def test_different_metadata(self) -> None:
        ...
        self.assertNotEqual(self.tf_original, tf_different_metadata)

        assert_frame_equal(self.tf_original.df, tf_different_metadata.df)
        self.assertEqual(self.tf_original.time_name, tf_different_metadata.time_name)
        self.assertEqual(self.tf_original.resolution, tf_different_metadata.resolution)
        self.assertEqual(self.tf_original.periodicity, tf_different_metadata.periodicity)
        self.assertEqual(self.tf_original.time_anchor, tf_different_metadata.time_anchor)
        self.assertEqual(self.tf_original._flag_manager, tf_different_metadata._flag_manager)
        self.assertNotEqual(self.tf_original.metadata, tf_different_metadata.metadata)
        self.assertEqual(self.tf_original.column_metadata, tf_different_metadata.column_metadata)
    ...
    def test_different_column_metadata(self) -> None:
        ...
        self.assertNotEqual(self.tf_original, tf_different_column_metadata)

        assert_frame_equal(self.tf_original.df, tf_different_column_metadata.df)
        self.assertEqual(self.tf_original.time_name, tf_different_column_metadata.time_name)
        self.assertEqual(self.tf_original.resolution, tf_different_column_metadata.resolution)
        self.assertEqual(self.tf_original.periodicity, tf_different_column_metadata.periodicity)
        self.assertEqual(self.tf_original.time_anchor, tf_different_column_metadata.time_anchor)
        self.assertEqual(self.tf_original._flag_manager, tf_different_column_metadata._flag_manager)
        self.assertEqual(self.tf_original.metadata, tf_different_column_metadata.metadata)
        self.assertNotEqual(self.tf_original.column_metadata, tf_different_column_metadata.column_metadata)
    ...
    def test_different_object(self, _: str, non_tf: Any) -> None:
        """Test that comparing against a non TimeSeries objects are not equal."""
        self.assertNotEqual(self.tf_original, non_tf)
